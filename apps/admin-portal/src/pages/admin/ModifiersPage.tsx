...
export default function ModifiersPage() {
  ...
  const handleSubmitModifier = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const displayNameValue = (modifierFormData.displayName || '').trim();
      const payload = {
        name: modifierFormData.name,
        displayName: displayNameValue || modifierFormData.name,
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Modifiers</h1>
          <p className="text-gray-600 text-sm">Manage modifier groups and options for your products</p>
        </div>
        <button
          onClick={() => handleOpenGroupModal()}
          className="bg-amber-700 text-white px-3 py-1.5 rounded-lg hover:bg-amber-800 transition flex items-center gap-2 text-sm"
        >
      ...
      {groups.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
          <i className="fas fa-sliders-h text-4xl text-gray-300 mb-3"></i>
          <h3 className="text-base font-semibold text-gray-900 mb-1">No modifier groups found</h3>
          <p className="text-gray-600 text-sm mb-3">Get started by creating your first modifier group</p>
          <button
            onClick={() => handleOpenGroupModal()}
            className="bg-amber-700 text-white px-3 py-1.5 rounded-lg hover:bg-amber-800 transition text-sm"
          >
            ...
            <div key={group.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              ...
              <div className="bg-gray-200 px-4 py-2 border-b border-gray-300">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <i className="fas fa-layer-group text-amber-700"></i>
                    <div>
                      <h3 className="font-semibold text-gray-900 text-sm">{group.displayName}</h3>
              ...
              <div className="px-4 py-2">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-medium text-gray-900 text-sm">Modifiers</h4>
                  <button
                    onClick={() => handleOpenModifierModal(group.id)}
                    className="text-amber-700 hover:bg-amber-50 px-2 py-0.5 rounded transition text-xs flex items-center gap-1"
                  >
                    <i className="fas fa-plus"></i>
                    Add Modifier
                  </button>
                ...
                {group.modifiers.length === 0 ? (
                  <div className="text-center py-4 text-gray-500">
                    ...
                    <p className="text-xs">No modifiers in this group</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
                    {group.modifiers.map(modifier => (
                      <div
                        key={modifier.id}
                        className="border border-gray-200 rounded p-2 hover:shadow-sm transition bg-gray-50"
                      >
                        <div className="flex items-start justify-between mb-1">
                          <div className="flex-1 min-w-0">
                            <h5 className="font-semibold text-base truncate" style={{ color: '#3f4c67' }} title={modifier.displayName}>{modifier.displayName}</h5>
                            <p className="text-xs text-gray-500 truncate" title={modifier.name}>{modifier.name}</p>
                          </div>
                          <span className={`ml-1 px-1.5 py-0.5 rounded text-[10px] font-medium flex-shrink-0 ${
                        ...
                        <div className="flex gap-1">
                          <button
                            ...
                            className="flex-1 px-1.5 py-0.5 text-[10px] bg-amber-700 text-white rounded hover:bg-amber-800 transition"
                          >
                            <i className="fas fa-edit mr-0.5"></i>
                            Edit
                          </button>
                          <button
                            onClick={() => handleDeleteModifier(modifier)}
                            className="px-1.5 py-0.5 text-[10px] bg-red-600 text-white rounded hover:bg-red-700 transition"
                          >
